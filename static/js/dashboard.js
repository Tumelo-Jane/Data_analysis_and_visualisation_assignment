...
const fetchSeries = (indicator, start, end, period) =>
  getJSON("/api/series", { indicator, start, end, period });
const fetchPairs = (x, y, start, end, period) =>
  getJSON("/api/pairs", { x, y, start, end, period });
...
const unionYears = (a, b) =>
  Array.from(new Set([...(a || []), ...(b || [])])).sort((x, y) => x - y);

const seriesToAligned = (s, years) => {
  const m = new Map((s.years || []).map((y, i) => [y, s.values[i]]));
  return years.map((y) => (m.has(y) ? m.get(y) : null));
};

const yoyToAligned = (s, years) => {
  const m = new Map((s.years || []).map((y, i) => [y, s.yoy[i]]));
  return years.map((y) => (m.has(y) ? m.get(y) : null));
};

const csvFromFiltered = (years, aVals, bVals, aName, bName) =>
  [
    ["Year", aName, bName],
    ...years.map((y, i) => [y, aVals[i] ?? "", bVals[i] ?? ""]),
  ]
    .map((r) => r.join(","))
    .join("\n");
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (!applyBtn || !resetBtn) return; // not the Plotly page
  applyBtn.addEventListener("click", () => apply().catch(console.error));
